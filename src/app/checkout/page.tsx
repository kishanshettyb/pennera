...
import { useRemoveFromCart } from '@/services/mutation/cart/cart'
...
function CheckoutPage() {
  ...
  const [updatingItems] = React.useState<Set<string>>(new Set())
  const [removingItems] = React.useState<Set<string>>(new Set())
  ...
  // Get applied coupon from cart data
  const appliedCouponFromCart = React.useMemo(() => {
    if (cart?.coupons && cart.coupons.length > 0) {
      return cart.coupons[0].code
    }
    return ''
  }, [cart?.coupons])
  ...
  const form = useForm<CheckoutFormData>({
    defaultValues: {
      ...
      selected_coupon: appliedCouponFromCart || ''
    },
    mode: 'onChange'
  })
  ...
  // Initialize applied coupon from cart data on component mount
  React.useEffect(() => {
    if (appliedCouponFromCart && !appliedCoupon) {
      setAppliedCoupon(appliedCouponFromCart)
      form.setValue('selected_coupon', appliedCouponFromCart)
    }
  }, [appliedCouponFromCart, appliedCoupon, form])
  ...
  }, [checkoutData, form, defaultPaymentMethod, appliedCouponFromCart])
                      ...
                      >
                        <div className="w-16 h-16 relative">
                          ...
                          >
                            ...
                            />
                            <div className="absolute -top-2 bg-red-600 text-white text-xs font-semibold -right-2 w-[20px] flex justify-center items-center rounded-2xl h-[20px]">
                              {item.quantity}
                            </div>
                          </div>
                        </div>
                        <div className="flex-1 flex flex-col justify-between h-full">
                          ...
                          {/* <div className="flex hidden items-center gap-2 mt-2">
                            <Counter
                              className="text-xs"
                ...
                {totalDiscount > 0 && (
                  <div className="px-4 py-2 flex justify-between items-center">
                    <div>
                      ...
                      {appliedCoupon && (
                        <p className="text-xs text-gray-500">Coupon: {appliedCoupon}</p>
                      )}
                    </div>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                              {availableCoupons.map((coupon: Coupon) => (
                                <Button
                                  ...
                                  className="justify-start text-left gap-5 flex h-auto py-2"
                                >
                                  <div className="flex flex-row gap-5 items-center">
                                    <span className="text-md font-semibold flex justify-start gap-2  items-center">
                                      <Gift />
                                      {coupon.code}
                                    </span>
